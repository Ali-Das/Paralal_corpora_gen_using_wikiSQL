...
class DataConversionUtil:
    ...
    def build_table_mapping(self, dataset):
        ...
        tables = pd.read_json("data/WikiSQL_data/" + dataset + ".tables.jsonl", lines=True)
        data = pd.DataFrame()
        for index, line in tables.iterrows():
            ...
            data = pd.concat([data, line_df])
        self.save_dataframe(data, "data/WikiSQL_data/tokenized_" + dataset + ".tables.jsonl")

    def get_query_from_json(self, json_line):
        """Returns a Query object for the json input and returns the table object as well"""
        sql = Query.from_dict(json_line["sql"])
        t_id = json_line["table_id"]
        table = self.table_map[t_id]
        table = Table("", table["header"], table["types"], table["rows"])
        return table, sql
    ...
    def build_tokenized_dataset(self, dataset):
        ...
        queries = pd.read_json("data/WikiSQL_data/" + dataset + ".jsonl", lines=True)
        count = 0
        ...
        for index, line in queries.iterrows():
            ...
            # append query to dict, to add it to the dataframe
        ...
        self.save_dataframe(data, "data/WikiSQL_data/tokenized_" + dataset + ".jsonl")
