...
class ParallelCorporaGenUtil:
    ...
    def build_parallel_corpora_from_json(self, dataset):
        ...
        queries = pd.read_json("data/wikisql_data/tokenized_" + dataset + ".jsonl", lines=True)
        tables = pd.read_json("data/wikisql_data/tokenized_" + dataset + ".tables.jsonl", lines=True)
        # iterate over the queries and convert each one to plain text sql
        filename_tgt = "data/tgt-" + dataset + ".txt"
        filet = open(filename_tgt, "a+", encoding="utf-8")
        filename_src = "data/src-" + dataset + ".txt"
        files = open(filename_src, "a+", encoding="utf-8")
